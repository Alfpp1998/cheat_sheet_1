...
$T_n = \sqrt{n} \max_{1 \le i \le n}\\
\left\{
\max \left( 
\left| \frac{i-1}{n} - F^0(X_{(i)}) \right|,
\left| \frac{i}{n} - F^0(X_{(i)}) \right|
\right)
\right\}.$
Or, in compact form,

$T_n = \sup_{t \in \mathbb{R}} \left| F_n(t) - \Phi_{\hat{\mu}, \hat{\sigma}}(t) \right|.$

They do not depend on unknown parameters!\\
This is the \textbf{Kolmogorov--Lilliefors test}.
