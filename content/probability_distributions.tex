...
$\mathbb{E}[X]=np$\\
$Var(X)= np(1-p)$ \\
$\text{Mode} = \lfloor (n+1)p \rfloor,\quad$ \\
$\text{Median} \approx \lfloor np \rfloor \text{ or } \lceil np \rceil$\\
Likelihood:\\
...
MLE:\\
Resolviendo para $p$:\\
$\displaystyle \hat{p}_{MLE} = \dfrac{\sum_{i=1}^n x_i}{nN}$\\
(o si se trata de una sola muestra con $x$ éxitos en $n$ ensayos:)\\
$\displaystyle \hat{p}_{MLE} = \dfrac{x}{n}$\\[6pt]
Fisher Information:\\
$I(p) = \frac{n}{p(1-p)}$\\
Canonical exponential form:\\
$f_ p(y) =\\  
exp [y \underbrace{(\ln (p)-\ln (1-p))}_{\theta } + \underbrace{n\ln (1-p)}_{-b(\theta )} \\
+\underbrace{\ln(\binom {n}{y})}_{c(y,\phi )} ]$
...
$F(k; \lambda) = P(X \leq k) = \sum_{i=0}^{k} \frac{e^{-\lambda} \lambda^{i}}{i!}$\\
$\mathbb{E}[X]=\lambda$\\
$Var(X)=\lambda$\\
$\text{Mode} = \lfloor \lambda \rfloor,\quad$\\
$\text{Median} \approx \lfloor \lambda + 1/3 - 0.02/\lambda \rfloor$\\
...
$F(k; p) = P(X \leq k) = 1 - (1 - p)^k$\\
$\mathbb{E}[T]=\frac{1}{p}$\\
$var(T)=\frac{1-p}{p^2}$\\
$\text{Mode} = 1,\quad$ \\
$\text{Median} = \left\lceil \frac{-1}{\ln(1-p)} \right\rceil$\\
...
$\text{Mode} = 0,\quad \text{Median} = \frac{\ln(2)}{\lambda}$
...
$T_i \sim iid \sim Geometric(p)$\\

$\mathbb{E}[Y_k]=\frac{k}{p}$\\

$Var(Y_k)= \frac{k(1-p)}{p}$\\
$\text{Mode} = \left\lfloor \frac{(r-1)(1-p)}{p} \right\rfloor,\quad$\\
$\text{Median} \approx \frac{r(1-p)}{p}$\\


$p_{Y_k}(t) ={t-1 \choose k-1}p^k(1-p)^{t-k}$\\
$F(k; r, p) = P(X \leq k) = \sum_{i=r}^{k} \binom{i-1}{r-1} p^{r} (1 - p)^{i - r}$\\
...
Parameters: shape $k \in \mathbb{N}$, rate $\lambda > 0$, continuous\\
$ 
f_x(x)=
	\begin{cases}
		 \dfrac{\lambda^k x^{k-1} e^{-\lambda x}}{(k-1)!}, & \text{if } x \ge 0\\
		0, & \text{o.w.}
	\end{cases}
$\\
...
F_x(x)=
	\begin{cases}
		 1 - e^{-\lambda x}\sum_{i=0}^{k-1}\dfrac{(\lambda x)^i}{i!}, & \text{if } x \ge 0\\
		0, & \text{o.w.}
	\end{cases}
$\\
...
$\text{Mode} = \frac{(\alpha-1)}{\lambda},\quad \text{Median} \approx \frac{\alpha - 1/3}{\lambda}$

Likelihood:\\
$L(X_1\dots X_n;\lambda)=
\dfrac{\lambda^{nk}e^{-\lambda\sum_{i=1}^n X_i}\prod_{i=1}^n X_i^{k-1}}
{((k-1)!)^n}$\\

Loglikelihood:\\
$\ell_n (\lambda)=nk\ln(\lambda)-\lambda\sum_{i=1}^n X_i\\
+(k-1)\sum_{i=1}^n\ln(X_i)-n\ln((k-1)!)$\\

MLE:\\
$\hat{\lambda}_{MLE}= \dfrac{nk}{\sum^{n}_{i=1}X_i}$\\

Fisher Information:\\
$I(\lambda)= \dfrac{k}{\lambda^2}$\\

Canonical exponential form:\\
$f_{\theta}(y) = 
\exp\Big(k\theta + (k-1)\ln y - e^{\theta}y - \ln((k-1)!)\Big)$\\
$\theta = \ln(\lambda)$\\
$\phi = 1$\\
...
$Var(X)=\frac{(b-a)^2}{12}$\\
$\text{Mode} = \text{any value in } [a,b],\quad$ 
...
$\ell(a,b)=
	\begin{cases}
		-n \ln(b - a), & a \le \min(X_i), \; b \ge \max(X_i)\\
		-\infty, & \text{o.w.}
	\end{cases}
$\\[6pt]

MLE:\\
$\hat{a}_{MLE} = \min(X_i)$,\quad
$\hat{b}_{MLE} = \max(X_i)$\\[6pt]

Fisher Information:\\
\text{undefined (depends on support limits)}\\

\subsection*{Gamma}
Parameters $\alpha, \lambda$, continuous\\
$ f_X(x)=
	\begin{cases}
		\dfrac{\lambda^{\alpha} x^{\alpha-1} e^{-\lambda x}}{\Gamma(\alpha)}, & \text{if } x \ge 0 \\
		0, & \text{o.w.}
	\end{cases}
$\\[6pt]
$F_X(x)= P(X \le x)= \dfrac{1}{\Gamma(\alpha)} \gamma(\alpha, \lambda x)$\\
(where $\gamma(\alpha, \lambda x)$ is the lower incomplete Gamma function)\\[6pt]
$\mathbb{E}[X]=\dfrac{\alpha}{\lambda}$\\
$\mathbb{E}[X^2]=\dfrac{\alpha(\alpha+1)}{\lambda^2}$\\
$Var(X)=\dfrac{\alpha}{\lambda^2}$\\
$\text{Mode} = \frac{(\alpha - 1)}{\lambda} \text{ for } \alpha > 1,\quad$\\
$\text{Median} \approx \frac{\alpha - 1/3}{\lambda}$\\

Likelihood:\\
$L(X_1,\dots,X_n;\alpha,\lambda)=
	\dfrac{\lambda^{n\alpha}}{\Gamma(\alpha)^n}
	\prod_{i=1}^n x_i^{\alpha-1} e^{-\lambda x_i}$\\[6pt]
Loglikelihood:\\
$\ell_n(\alpha,\lambda)= n\alpha \ln(\lambda) - n \ln(\Gamma(\alpha))
+ (\alpha-1)\sum_{i=1}^n \ln(x_i) - \lambda \sum_{i=1}^n x_i$\\[6pt]
MLE for $\lambda$ (given $\alpha$):\\
$\hat{\lambda}_{MLE} = \dfrac{n\alpha}{\sum_{i=1}^n x_i}$\\[6pt]
Fisher Information (for $\lambda$):\\
$I(\lambda)=\dfrac{\alpha}{\lambda^2}$\\[6pt]
Canonical exponential form:\\
$f_{\theta}(y)=\exp\big(\underbrace{y\theta}_{\text{sufficient stat.}} - 
\underbrace{(-\alpha\ln(-\theta))}_{b(\theta)} + 
(\alpha-1)\ln(y) - \ln\Gamma(\alpha)\big)$\\
$\theta = -\lambda,\quad \phi=1$\\

\subsection*{Beta}
Parameters $\alpha, \beta$, continuous\\
$ f_X(x)=
	\begin{cases}
	...
	\end{cases}
$\\[6pt]
$F_X(x)= P(X \le x)= I_x(\alpha,\beta)$\\
(where $I_x(\alpha,\beta)$ is the regularized incomplete Beta function)\\[6pt]
$\mathbb{E}[X]=\dfrac{\alpha}{\alpha+\beta}$\\
$\mathbb{E}[X^2]=\dfrac{\alpha(\alpha+1)}{(\alpha+\beta)(\alpha+\beta+1)}$\\
$Var(X)=\dfrac{\alpha\beta}{(\alpha+\beta)^2(\alpha+\beta+1)}$\\
$\text{Mode} = \frac{\alpha - 1}{\alpha + \beta - 2} \text{ for } \alpha,\beta > 1,\quad$ 
$\text{Median} \approx \frac{\alpha - 1/3}{\alpha + \beta - 2/3}$\\
Likelihood:\\
$L(X_1,\dots,X_n;\alpha,\beta)=
	\dfrac{1}{B(\alpha,\beta)^n}
	\prod_{i=1}^n x_i^{\alpha-1}(1-x_i)^{\beta-1}$\\[6pt]
Loglikelihood:\\
$\ell_n(\alpha,\beta)=
	-n\ln(B(\alpha,\beta))
	+ (\alpha-1)\sum_{i=1}^n \ln(x_i)
	+ (\beta-1)\sum_{i=1}^n \ln(1-x_i)$\\[6pt]
Fisher Information Matrix entries (for large $n$):\\
$I_{\alpha\alpha} = n[\psi'(\alpha) - \psi'(\alpha+\beta)]$,\\
$I_{\beta\beta} = n[\psi'(\beta) - \psi'(\alpha+\beta)]$,\\
$I_{\alpha\beta} = -n\psi'(\alpha+\beta)$\\
(where $\psi'$ is the trigamma function)\\[6pt]
Canonical exponential form:\\
$f_{\theta}(y) =
\exp\big(
	y_1\theta_1 + y_2\theta_2 - b(\theta)
\big)$\\
with $\theta_1=\alpha-1$, $\theta_2=\beta-1$, and
$b(\theta)=\ln B(\alpha,\beta)$\\
